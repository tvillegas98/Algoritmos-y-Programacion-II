...
bool _abb_in_order(abb_nodo_t* raiz, bool visitar(const char* clave, void* dato, void* extra), void* extra){
    ...
    if(!ok) return false;
    ok = visitar(raiz->clave, raiz->dato, extra);
    if(!ok) return false;
...
bool _abb_iterar_en_rango(abb_nodo_t* raiz, abb_comparar_clave_t cmp, bool visitar(const char*, void*, void*), void* extra, const char* inicio, const char* fin){
    if(!raiz) return true;
    bool ok = true;

    if(cmp(inicio, NULO) == 0 || cmp(raiz->clave, inicio) >= 0){
        ok = _abb_iterar_en_rango(raiz->izq, cmp, visitar, extra,inicio, fin);
    }

    if((cmp(raiz->clave, inicio) >= 0 && cmp(raiz->clave, fin) <= 0) || (cmp(raiz->clave, inicio) >= 0 && cmp(fin, NULO) == 0)){
        if(!visitar(raiz->clave, raiz->dato, extra) && !ok) return false;
    }

    if(ok && (cmp(fin, NULO) == 0 || cmp(raiz->clave, fin) <= 0)){
        _abb_iterar_en_rango(raiz->der, cmp, visitar, extra,inicio, fin);
    }
    return ok;
}

void abb_iterar_en_rango(abb_t* arbol, bool visitar(const char*, void*, void*), void* extra, const char* inicio, const char* fin){
    _abb_iterar_en_rango(arbol->raiz, arbol->comparador, visitar, extra, inicio, fin);
}
...
//     abb_guardar(arbol, "Hipocrates", NULL);
//     abb_guardar(arbol, "Cortez", NULL);
//     abb_guardar(arbol, "Freitag", NULL);
//     abb_guardar(arbol, "Anton", NULL);
//     abb_iterar_en_rango(arbol, imprimir, NULL, "Freitag", "Hipocrates");
// }